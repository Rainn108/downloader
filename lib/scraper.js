...
async function ymCDN(url, format = 'mp3') {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // Format di sini bakal nerima input '1080', '720', atau '320' dari fungsi youtubeDl
...
async function youtubeDl(url) {
    try {
        // --- BAGIAN OTOMATIS RESOLUSI TERTINGGI ---
        // Kita coba tembak 1080 dulu, kalau gagal turun ke 720, dst.
        const videoQualities = ['1080', '720', 'mp4']; 
        let bestVideo = { status: false };
        
        for (let q of videoQualities) {
            const attempt = await ymCDN(url, q);
            if (attempt.status) {
                bestVideo = attempt;
                bestVideo.qualityLabel = (q === 'mp4') ? 'Default' : q + 'p';
                break; // Kalau dapet yang tinggi, langsung stop looping
            }
        }

        // --- BAGIAN AUDIO TERBAIK ---
        // Coba tembak bitrate 320
        const bestAudio = await ymCDN(url, '320');

        const results = [];
        if (bestVideo.status) {
            results.push({ quality: bestVideo.qualityLabel, type: 'mp4', url: bestVideo.dl });
        }
        if (bestAudio.status) {
            results.push({ quality: '320kbps', type: 'mp3', url: bestAudio.dl });
        } else {
            // Fallback kalau 320 gagal, ambil mp3 biasa
            const normalAudio = await ymCDN(url, 'mp3');
            if (normalAudio.status) results.push({ quality: '128kbps', type: 'mp3', url: normalAudio.dl });
        }

        const primary = bestVideo.status ? bestVideo : bestAudio;
        return {
            title: primary.title,
