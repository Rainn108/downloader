...
async function ymCDN(url, format = 'mp3') {
    return new Promise(async (resolve, reject) => {
        ...
        if (!isYouTubeUrl.test(url)) return resolve({ status: false, mess: "URL tidak valid." });
        ...
        try {
            ...
            if (!init.data.convertURL) return resolve({ status: false });
            
            // Format di sini dinamis (bisa '1080', '720', '320', dll)
            const convert = await axios.get(`${init.data.convertURL}&v=${id}&f=${format}&_=${Math.random()}`, { headers: hr }).then(x => x.data);
            
            if (!convert.progressURL || !convert.downloadURL) return resolve({ status: false });
            
            let currentProgress = 0;
            let title = '';
            while (currentProgress < 3) {
                const response = await axios.get(convert.progressURL, { headers: hr });
                if (response.data.error > 0) return resolve({ status: false });
                currentProgress = response.data.progress;
                title = response.data.title;
                if (currentProgress < 3) await new Promise(res => setTimeout(res, 500));
            }
...
// 2. FUNGSI LOGIKA (AUTO-BEST RESOLUTION)
async function youtubeDl(url) {
    try {
        // Daftar resolusi yang mau dicoba dari yang paling tinggi
        const videoPriority = ['1080', '720', '480', '360', 'mp4'];
        const audioPriority = ['320', '256', '128', 'mp3'];

        let bestVideo = null;
        let bestAudio = null;

        // Cek video tertinggi sampai ketemu satu yang berhasil
        for (let q of videoPriority) {
            const res = await ymCDN(url, q);
            if (res.status) {
                bestVideo = { quality: q === 'mp4' ? 'Default' : q + 'p', type: 'mp4', url: res.dl, title: res.title, id: res.id };
                break; 
            }
        }

        // Cek audio tertinggi sampai ketemu satu yang berhasil
        for (let b of audioPriority) {
            const res = await ymCDN(url, b);
            if (res.status) {
                bestAudio = { quality: b === 'mp3' ? '128kbps' : b + 'kbps', type: 'mp3', url: res.dl };
                break;
            }
        }

        const results = [];
        if (bestVideo) results.push(bestVideo);
        if (bestAudio) results.push(bestAudio);

        return {
            title: bestVideo?.title || bestAudio?.title || "YouTube Media",
            preview: `https://i.ytimg.com/vi/${(bestVideo?.id || bestAudio?.id)}/hqdefault.jpg`,
            results: results
        };
    } catch (e) {
        throw new Error("Gagal dapet resolusi terbaik, Bang.");
    }
}
    id: meta.id,
